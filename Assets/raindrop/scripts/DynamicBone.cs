...
public class DynamicBone : MonoBehaviour
{
    ...
    public List<Transform> m_Connection = null;
    ...
    class Particle
    {
        ...
        public List<Particle> ref_Particles = null;
    }

    List<Particle> m_Particles = new List<Particle>();
    List<List<Particle>> m_refParticles = new List<List<Particle>>();
    void Start()
    {
        SetupParticles();
    }
    ...
    void SetupParticles()
    {
        ...
        {
           // ref_Particles = new List<Particle>();//m_refParticles[i];//like same level nodes
        }
        ...
        //Transform 
        if(0 == m_Connection.Count)
            AppendParticles(m_Root, null, -1, 0);
        else
            AppendParticles(m_Root, m_Connection, -1, 0);
    ...
    void ParticleValued(ref Particle p, int parentIndex)
    {
        if (p.m_Transform != null)
        {
            p.m_Position = p.m_PrevPosition = p.m_Transform.position;
            p.m_InitLocalPosition = p.m_Transform.localPosition;
            p.m_InitLocalRotation = p.m_Transform.localRotation;
        }
        else 	// end bone
        {
    ...
    void AppendParticles(Transform b, List<Transform> refNodes,int parentIndex, float boneLength)//same level for refNodes
    {
        Particle p = new Particle();
        p.m_Transform = b;
        p.m_ParentIndex = parentIndex;
        ParticleValued(ref p, parentIndex);
        if (parentIndex >= 0)
        {
        ...
        if (null != refNodes && refNodes.Count > 0)
        {
            if (null == p.ref_Particles)
                p.ref_Particles = new List<Particle>();//m_refParticles[i];//like same level nodes
            for (int i = 0; i < refNodes.Count; i++)
            {
                Particle pref = new Particle();
                pref.m_Transform = refNodes[i];
                pref.m_ParentIndex = parentIndex;

                ParticleValued(ref pref, parentIndex);

                p.ref_Particles.Add(pref);
            }
        }
        ...
        if (b != null)
        {
            for (int i = 0; i < b.childCount; ++i)
            {
                ...
                if (!exclude)
                {
                    List<Transform> childNodes = null;
                    if (null != refNodes)
                    {
                        childNodes = new List<Transform>();
                        Transform c = null;
                        for (int j = 0; j < refNodes.Count; j++)
                        {
                            c = refNodes[j].GetChild(i);
                            childNodes.Add(c);
                        }
                    }

                    AppendParticles(b.GetChild(i), childNodes,index, boneLength);
                }
                    
            }

            if (b.childCount == 0 && (m_EndLength > 0 || m_EndOffset != Vector3.zero))
                AppendParticles(null, null,index, boneLength);
        }
    }
    ...
    void UpdateParticles1()
    {
        ...
        for (int i = 0; i < m_Particles.Count; ++i)
        {
            ...
            if (p.m_ParentIndex >= 0)
            {
                ...
                {
                    for (int j = 0; j < p.ref_Particles.Count; j++)
                    {
                        p.ref_Particles[j].m_Position += v * (1 - p.m_Damping) + force + rmove;
                    }
                }
            }
            else
    ...
    void UpdateParticles2()
    {
        ...
        for (int i = 1; i < m_Particles.Count; ++i)
        {
            ...
            if (stiffness > 0 || p.m_Elasticity > 0)
            {
                ...
                //for (int j = 0; j < p.ref_Particles.Count; j++)
                //{
                //    p.ref_Particles[j].m_Position += d * p.m_Elasticity;
                //}
                if (stiffness > 0)
                {
                    ...
                    if (len > maxlen)
                    {
                        ...
                        //for (int j = 0; j < p.ref_Particles.Count; j++)
                        //{
                        //    p.ref_Particles[j].m_Position += d * ((len - maxlen) / len);
                        //}
                    }
            ...
            if (m_Colliders != null)
            {
                ...
                for (int j = 0; j < m_Colliders.Count; ++j)
                {
                    ...
                    if (c != null && c.enabled)
                    {
                        ...
                        //collide ref nodes here
                        if (m_Connection != null && m_Connection.Count != 0)
                        {
                            for (int k = 0; k < p.ref_Particles.Count; k++)
                            {
                                c.Collide(ref p.ref_Particles[k].m_Position, particleRadius);
                            }
                        }
            ...
            if (leng > 0)
            {
                ...
                if (m_Connection != null && m_Connection.Count != 0)
                {
                    for (int j = 0; j < p.ref_Particles.Count; j++)
                    {
                        p.ref_Particles[j].m_Position += dd * ((leng - restLen) / leng);
                    }
                }
            }   
        }
